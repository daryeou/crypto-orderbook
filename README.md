...
|---|---|---|---|---|
| Upbit | 가능 | 높음 | 양호 | KRW 마켓 제공 |
| Bithumb | 가능 | 높음 | 보통 | 응답 구조 해석 비용이 더 큼 |
| Binance | 가능 | 제한적 | 우수 | 국내 검증 환경이 불리할 수 있음 |
...
- `market/all`, `ticker`, `orderbook`, `ticker` 조합으로 과제 요구사항을 단순하게 충족할 수 있습니다.
...
- 이전에 진행했던 MVVM 기반 토이 프로젝트의 모듈 분리 경험은 참고하되, `core:designsystem`, `core:common` 같은 확장 레이어는 도입하지 않았습니다.
...
- 종목 리스트: `MarketListUiState`
- 호가창: `OrderBookUiState` + `ConnectionState`
- 각 화면은 `Route(hiltViewModel())`와 `Screen(uiState, onAction...)`로 분리
- `MarketListViewModel`은 REST polling, `OrderBookViewModel`은 WebSocket 구독을 담당
...
- 화면 이탈 시 ViewModel이 구독을 해제합니다.
- 백프레셔는 UI 소비 지점에서 다룹니다.

### Navigation 전략

- 앱 네비게이션은 Navigation3의 `rememberNavBackStack`과 `NavDisplay`를 사용합니다.
- destination key는 `@Serializable` 기반으로 구성합니다.
- `navigation3-runtime`, `navigation3-ui`, `lifecycle-viewmodel-navigation3` 조합을 사용합니다.
...
| Navigation3 | 화면 이동 | serializable key 기반 back stack과 상태 복원에 적합 |
| Hilt | DI | domain/data/network binding을 단순화 |
...
| Coroutines / Flow | 비동기 처리 | StateFlow, callbackFlow, polling 흐름 구성에 적합 |

## 가정과 판단

- 종목 리스트는 KRW 마켓만 표시합니다.
- 종목 리스트의 현재가와 24h 변동률은 REST `/v1/ticker`를 5초 polling으로 갱신합니다.
