...
사실 Upbit와 Bithumb은 핵심 기능(KRW 마켓, 한글 종목명, WebSocket `orderbook`/`ticker`)이 거의 동등합니다. 둘 중 어느 쪽을 골라도 과제 요구사항은 충족할 수 있는 구조였습니다.

Upbit를 선택한 이유는 두 가지입니다.

1. **호가 단 수를 유연하게 받을 수 있어 화면 설계 자유도가 높았다.** Upbit는 `orderbook_units` 파라미터로 5/10/15/30단 중 선택해 받을 수 있고, 이번 과제에서는 15단을 기본으로 두고 더 다채로운 호가 표현이 가능하도록 여지를 남겨 뒀습니다. Bithumb은 30단 고정이라 화면에서 잘라 표시해야 합니다.
2. **국내 거래량 1위로 사용자 환경과 가장 가깝다.** 실시간 호가가 가장 활발하게 움직이는 거래소에서 검증하면 UI 갱신 정책(샘플링 100ms, depth 표현 등)을 실제 부하 기준으로 잡기 좋습니다.
